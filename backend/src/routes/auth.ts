...
// Reset password using a token from an admin-generated reset link.
// The token is consumed (invalidated) only once a new password is successfully set.
router.post('/reset-password', async (req: Request, res: Response) => {
  try {
    ...
    const resetToken = await passwordResetTokenQueries.findValidByHash(tokenHash);
    ...
    await passwordResetTokenQueries.markUsed(resetToken.id);
